...
class Model(nn.Module):
    ...
    def __init__(self, depth=17, n_channels=64, image_channels=3, use_bnorm=True, kernel_size=3):
        ...
        padding = 1
        layers = []
        #layers.append(nn.Conv2d(in_channels=image_channels, out_channels=5*n_channels, kernel_size=kernel_size, padding=padding, bias=True))
        layers.append(nn.Conv2d(in_channels=image_channels, out_channels=128, kernel_size=kernel_size, padding=padding, bias=True))
        layers.append(nn.ReLU(inplace=True))
        #layers.append(nn.Conv2d(in_channels=5*n_channels, out_channels=1, kernel_size=1, padding=0, bias=True))
        layers.append(nn.Conv2d(in_channels=128, out_channels=1, kernel_size=1, padding=0, bias=True))
        layers.append(nn.ReLU(inplace=True))
        self.fea_ext = nn.Sequential(*layers)
        ...
        self.recons = nn.Sequential(*layers)
