...
#from models import xDnCNN as Model
...
parser.add_argument('--train_dir', dest='train_dir',default='../DRED/data/Train400', help="Train data dir")
#parser.add_argument('--train_dir', dest='train_dir', default='./data/train', help="Train data dir")
parser.add_argument('--test_dir', dest='test_dir', default='/home/lsy194593/dataset/Set12', help='Test data dir')
...
def train(training_data_loader, optimizer, model, criterion, epoch):
    ...
    for i, batch in enumerate(training_data_loader, 1):
        ...
        with torch.no_grad():
            ...
            noise_type,noise_level = est(noise_image)
            _,noise_type = torch.max(noise_type,1)
            noise_type = noise_type +1 

            noise_level = noise_level.view(-1,1,1,1).type_as(batch[0])
            noise_level = torch.ones(noise_image.size()).mul_(noise_level)
            noise_level = noise_level.cuda()

            noise_type = noise_type.view(-1,1,1,1).type_as(batch[0])
            noise_type = torch.ones(noise_image.size()).mul_(noise_type)
            noise_type = noise_type.cuda()
...
#-------------------------------------------------------------------------------------------------           
    #vis=visdom.Visdom()
    with torch.no_grad():
        for i,(image, target,noise_type_ori,noise_level_ori) in enumerate(test_loader):
            image_var = image
            target_var = target
            ...
            est_time = time.time()
            est_type,noise_level = est(image_var)
            print("time is %lf"%(time.time()-est_time))
            _,noise_type = torch.max(est_type,1)
            noise_type = noise_type +1 

            noise_level = noise_level[0][noise_type-1]
            ...
            est_time = time.time()
            clean_image,mask_t = model(mul_input)
            print("time is %lf"%(time.time()-est_time))
    ...
    #_,_,w,h=mask_t.size()
    #mask_t =mask_t.view(64,1,w,h).data.cpu().numpy()
    #for i in range(64):
    #    vis.image(mask_t[i:i+1]*255,opts={'title':'multi-image',})
